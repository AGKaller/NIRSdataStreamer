...
%             };
    ...
    defFile = fullfile(pth,'+StO2patchDefs',sprintf('%s.m',cfgname));
    if ~exist(defFile,'file')
        error('''%s'' does not exist in package ''StO2patchDefs''!',cfgname);
    end
    fh = str2func(cfgname);
    [Optodes, Types] = fh();
end
